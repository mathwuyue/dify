        ...
        if len(prompt_messages) > 0 and isinstance(
            ...
            prompt_messages.insert(
                ...
                SystemPromptMessage(
                    content=block_prompts.replace(
                        "{{instructions}}",
                        f"Please output a valid {code_block} with markdown codeblocks.",
                    )
                ),
            )
            ...
            if resp_finish_reason is not None and resp_finish_reason != "null":
                ...
                try:
                    if (
                        ...
                        resp_content = response.output.choices[
                            0
                        ].message.reasoning_content
                        if not is_reasoning_begin:
                            resp_content = "<think>" + resp_content
                            is_reasoning_begin = True
                    elif (
                        response.output.choices[0].message.content != ""
                        and response.output.choices[0].message.reasoning_content == ""
                    ):
                        resp_content = response.output.choices[0].message.content
                        if is_reasoning_begin and not is_reasoning_end:
                            resp_content = "</think>" + resp_content
    ...
    def _convert_tools(self, tools: list[PromptMessageTool]) -> list[dict]:
        ...
        for tool in tools:
            ...
            for p_key, p_val in properties.items():
                ...
                if "enum" in p_val:
                    desc += (
                        f"; Only accepts one of the following predefined options: "
                        f"[{', '.join(p_val['enum'])}]"
                    )
